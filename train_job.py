...
def load_sql_Files(sql_list: list):
    ...
    for i in range(0, len(sql_list)):
        sqlFiles = os.path.join(config.JOB_DATA_PATH, sql_list[i] + '.sql')
...
def getGMRL(sqls, modellist, pg_latency, nodeFeaturizer, costCache, workload, exp=None, old=None):
    ...
    for i in sqls:
        join_graph, all_join_conds, query_leaves, origin_dp_tables = DP.getPreCondition(
            os.path.join(config.JOB_DATA_PATH, i + '.sql'))
...
def getModelsFromFile(maxLevel, modelpath, device=None):
    ...
    for i in range(2, maxLevel + 1):
        ...
        if not os.path.exists(full_model_path):
            print(f'[DP-{i}]\tNo checkpoint from {full_model_path}')
...
def generateLatencyPool(fileName=config.LATENCY_FILE):
    """
    :param fileName:
    :return:
    """
...
def setInitialTimeout(sqls: list, dropbuffer, testtime=3):
    ...
    """
    """
    增加latency_file, 避免sql重复执行
    """
    timeoutlist = []
    latency_handler, LatencyDict = generateLatencyPool()
    for i in sqls:
        if i in LatencyDict:
            timeout = LatencyDict[i]
        else:
            tem1 = 0
            ...
            latency_handler.write(json.dumps([i, timeout])+"\n")
            latency_handler.flush()
        timeoutlist.append(round(timeout, 3))
    latency_handler.close()
    return timeoutlist
        ...
        if i in LatencyDict:
            ...
            for j in range(0, 3):
                tem1 = tem1 + postgres.GetLatencyFromPg(i, None, verbose=False, check_hint_used=False, timeout=90000,
                                                        dropbuffer=False)
            latency = tem1 / 3.0
            latency_handler.write(json.dumps([i, latency])+"\n")
            latency_handler.flush()
...
if __name__ == '__main__':
    ...
    if not config.DATABASE.lower() == 'imdb':
        raise ValueError(f"The current connection in 'config.py' does not point to the JOB database.")
    ...
    train_config = {'log_path': 'log'}
    timestamp = str(datetime.datetime.now().strftime(ISOTIMEFORMAT))
    loglogs = '_'.join((logs_name, timestamp))
    log_dir = os.path.join(train_config['log_path'], loglogs)
    os.makedirs(log_dir)
